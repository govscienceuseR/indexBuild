...
#' @export
#' @details NOTE: OpenAlex has deprecated Concepts in favor of Topics; prefer \code{\link{queryTopics}} for new work. The \code{/concepts} endpoint (and the \code{concept.id} works filter) still function as deprecated aliases. NOTE: https://api.openalex.org/concepts doesn't seem to tolerate regex at this point, so that needs to be done with the output.
#' @importFrom stringr str_detect
#' @importFrom jsonlite read_json
#' @import httr
#' @import data.table

queryConcepts <- function(concept_string = NULL,maxlevel = 10,per_page = 10,mailto = NULL,max_return = 100,api_key = NULL,variables = c('id','display_name','level','works_count','works_api_url','description')){
